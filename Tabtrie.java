...
public class Tabtrie {
	public boolean rechercheT(int v, int tabTrie[]){
		int m = 0;
		...
		while(x == false && (ifin - id) > 1){
		    ...
		    x = (tabTrie[m] == v);  //on regarde si la valeur recherchée est à cet indice
		    
		    if(tabTrie[m] > v) ifin = m;  //si la valeur qui est à la case "im" est supérieure à la valeur recherchée, l'indice de fin "ifin" << devient >> l'indice de milieu, ainsi l'intervalle de recherche est restreint lors du prochain tour de boucle
